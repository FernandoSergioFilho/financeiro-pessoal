...
function state(overrides: Partial<FinanceData> = {}): FinanceData {
  return {
    version: 1,
    accounts: [{ id: 'a1', name: 'Conta', kind: 'checking', openingBalance: 0, color: 'blue' }],
    categories: [{ id: 'c1', name: 'Casa', kind: 'expense', color: 'blue', emoji: '🏠' }],
    entries: [], recurring: [], purchases: [], ...overrides,
  };
}
...
describe('lançamentos', () => {
  it('cria, edita e apaga', () => {
    ...
    s = reducer(s, { type: 'entry/delete', id: 'e1' });
...
describe('ocorrências de contas recorrentes', () => {
  ...
  it('apagar um lançamento avulso não toca nas regras', () => {
    const s = reducer(state({ recurring: [rule()], entries: [entry()] }), { type: 'entry/delete', id: 'e1' });
...
describe('compras parceladas', () => {
  ...
  it('apagar a compra leva junto as parcelas dela, e só elas', () => {
    ...
    s = reducer(s, { type: 'purchase/delete', id: built.purchase.id });
  ...
  it('apagar uma parcela isolada mantém as demais', () => {
    ...
    s = reducer(s, { type: 'entry/delete', id: built.entries[0]!.id });
...
describe('contas', () => {
  it('exclui de vez a conta que nunca foi usada', () => {
    const s = reducer(state(), { type: 'account/delete', id: 'a1' });
    expect(s.accounts).toHaveLength(0);
  });

  it('arquiva, em vez de excluir, a conta com movimento', () => {
    const s = reducer(state({ entries: [entry()] }), { type: 'account/delete', id: 'a1' });
...
describe('categorias', () => {
  it('apagar a categoria deixa os lançamentos sem categoria, não os apaga', () => {
    const s = reducer(state({ entries: [entry()], recurring: [rule()] }), { type: 'category/delete', id: 'c1' });
